...
<?php
	...
	public static function enqueue_scripts()
	{
		...
		if (!wp_script_is($script_id, 'enqueued') && static::$post_type == $post_type) {
			$script_url =  Settings::PLUGIN_URL . implode(DIRECTORY_SEPARATOR, array('admin', 'assets', 'js', static::$post_type . '-admin.js'));
			$script_file = Settings::PLUGIN_DIR_PATH . implode(DIRECTORY_SEPARATOR, array('admin', 'assets', 'js', static::$post_type . '-admin.js'));
	...
	public static function enqueue_styles()
	{
		...
		if (!wp_style_is($style_id, 'enqueued') && static::$post_type == $post_type) {
			$style_url =  Settings::PLUGIN_URL . 'admin/css/' . static::$post_type . '-admin.css';
			$style_file = Settings::PLUGIN_DIR_PATH . 'admin/css/' . static::$post_type . '-admin.css';

			if (file_exists($style_file)) {
				wp_enqueue_style($style_id, $style_url, array(), Settings::PLUGIN_VERSION, 'all');
			}
		}
	}
	...
	static function post_type_template($single)
	{
		...
		if ($post->post_type === static::$post_type) {
			$plugin_dir_path    = Settings::PLUGIN_DIR_PATH;
